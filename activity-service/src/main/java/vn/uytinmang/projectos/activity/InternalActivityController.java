...
class InternalActivityController {
    private final ResourceApplicationService activities;
    private final byte[] internalToken;

    InternalActivityController(ResourceApplicationService activities,
    ...
    ApiResponse<JsonNode> create(@RequestHeader(value = "X-Internal-Token", required = false) String token,
        ...
        ObjectNode activity = body.deepCopy();
        activity.remove("projectId");
        activity.remove("actorId");
        activity.put("legacyId", eventId.toString());
        try {
            return ApiResponse.of(activities.create(projectId, "activities", activity, actorId));
        } catch (ApiException exception) {
            if (!"legacy_id_exists".equals(exception.code())) throw exception;
            return ApiResponse.of(activities.get(projectId, "activities", eventId.toString()));
        }
    }
