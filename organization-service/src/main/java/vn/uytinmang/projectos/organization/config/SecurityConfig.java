package vn.uytinmang.projectos.organization.config;
...
@Configuration("organizationSecurityConfig")
@ConditionalOnProperty(name = "app.monolith.enabled", havingValue = "false", matchIfMissing = true)
@EnableMethodSecurity
class SecurityConfig {
    @Bean
    JwtDecoder organizationJwtDecoder(@Value("${app.jwt.secret}") String secret) {
    ...
    @Bean
    SecurityFilterChain organizationSecurity(HttpSecurity http, JwtDecoder decoder,
        ...
        http.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/actuator/health", "/v3/api-docs/**", "/api/v1/internal/**").permitAll()
