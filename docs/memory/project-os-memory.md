...
- On 2026-08-27, V28–V29 completed indexes for every child-side foreign-key column set, including composite keys in FK order. Fresh PostgreSQL verification now targets Flyway V1–V29, 58 public tables, 102 validated foreign keys, zero orphan/tenant mismatches, and zero unindexed foreign keys.
...
- Public schema hardening is canonical through V27: V9 removes duplicate FKs and adds ownership constraints, V10–V12 normalize verified legacy UUID references, V13 adds the empty `organization_branches` owner table, V14–V22 add employee/account, settings, attendance-location, soft-delete, audit/integrity, offboarding, recycle-bin, performance-scoring and position-profile domains, V23–V25 add company assets/resources, salary regulations and notification categories, V26 adds candidate onboarding, and V27 adds tenant-safe organization UUID links for legacy contracts, KPI evaluations and leave balances. Original varchar business-code columns remain unchanged.
...
- Verification checkpoint 2026-08-25: full monolith reactor verification passed after the Operations typed-boundary and explicit-projection changes. Active Testcontainers suites report zero failures/errors; the subsequent V25 schema gate now covers 15 tests against PostgreSQL 17 with Flyway V1–V25.
...
- Fresh full verification on 2026-08-25 passed: `./mvnw -pl application/monolith-app -am verify` (all active reactor projects), `npm run typecheck`, `npm run architecture:check`, `npm run lint`, `npm run mock-data:check`, `npm run tanstack:check`, and `npm run build`. The default frontend build required one cache refresh through the Webpack build because a deleted `/accounts` route remained in the prior Turbopack cache; the subsequent default build passed.
- Schema hardening checkpoint on 2026-08-27: canonical monolith Flyway reaches V27. A fresh schema reports 58 public tables and 102 validated FKs; the release gate assertions cover the V27 schema contract. V27 adds only deterministic tenant links derived from validated employee UUIDs; no enterprise-to-enterprise FK is invented.
...
- Database checkpoint on 2026-08-27: a fresh Testcontainer applied Flyway V1–V27 successfully. The active Docker database remains at V26 until its backup, preflight and migration run are deliberately performed; production was not modified.
- Native database synchronization checkpoint on 2026-08-26: backup `/tmp/project-os-v25-before-native-sync.dump` was restored into PostgreSQL 18 at `127.0.0.1:5432/project_os`; user verification confirmed Flyway V25 and 56 public tables. The Docker PostgreSQL instance on port 15433 remains available as a local fallback.
...
- Database configuration cleanup checkpoint on 2026-08-27: the monolith Compose database now receives only `POSTGRES_DB`, `POSTGRES_USER` and `POSTGRES_PASSWORD`. The PostgreSQL init script no longer creates legacy domain roles or schemas. Module fallback configurations use unified `DB_USERNAME`/`POSTGRES_USER`, `DB_PASSWORD`/`POSTGRES_PASSWORD` and native port 5432; their old schema settings remain only for standalone module contract-test compatibility, while the active monolith owns `public`. Backend and frontend environment whitelists no longer expose the eight legacy password keys. Existing database catalogs were not modified.

- Environment Settings synchronization checkpoint on 2026-08-28: administrator environment configuration now uses a canonical monolith catalog, excludes gateway/frontend/local Compose/legacy S3/provider keys, merges process environment values with the configured file, masks runtime secrets, and rejects production updates and rollback. Frontend API/WebSocket variables remain frontend-owned; `POSTGRES_PASSWORD` and `MINIO_ROOT_*` remain local Compose-only.
- Environment contract unification checkpoint on 2026-08-28: local Compose now passes provider-neutral `OBJECT_STORAGE_*`, SMTP/email, Redis and 9Router runtime names to `monolith-app`; production Compose passes the same runtime contract and no longer exposes legacy `MINIO_*` application bindings. Native PostgreSQL remains an override layered on the base Compose file. SMTP is disabled in the production file until secret-manager credentials are supplied; local and production environment files remain intentionally separate.
