package com.projectos.backend.identity;
...
class AuthContractTest {
    ...
    @Test
    void rootAdminCanRecycleAndRestoreManagedUserWithinThirtyDays() throws Exception {
        var adminId = UUID.randomUUID();
        var admin = jwt().jwt(token -> token.claim("uid", adminId.toString()).claim("role", "ROOT_ADMIN"))
                .authorities(new SimpleGrantedAuthority("ROLE_ROOT_ADMIN"));
        String email = "recycle-" + UUID.randomUUID() + "@example.test";

        mvc.perform(post("/api/v1/admin/users").with(admin).contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"" + email + "\",\"password\":\"StrongPass123!\","
                                + "\"displayName\":\"Recycle User\",\"role\":\"USER\"}"))
                .andExpect(status().isCreated());
        var managed = users.findByEmail(email).orElseThrow();

        mvc.perform(delete("/api/v1/admin/users/" + managed.getId()).with(admin))
                .andExpect(status().isNoContent());
        mvc.perform(get("/api/v1/admin/users?status=DELETED").with(admin))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.meta.total").value(1))
                .andExpect(jsonPath("$.data[0].status").value("DELETED"))
                .andExpect(jsonPath("$.data[0].deletedAt").isNotEmpty())
                .andExpect(jsonPath("$.data[0].deleteExpiresAt").isNotEmpty())
                .andExpect(jsonPath("$.data[0].restoreDaysRemaining").value(org.hamcrest.Matchers.greaterThan(0)));
        mvc.perform(post("/api/v1/auth/login").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"" + email + "\",\"password\":\"StrongPass123!\"}"))
                .andExpect(status().isUnauthorized());

        mvc.perform(post("/api/v1/admin/users/" + managed.getId() + "/restore").with(admin))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.status").value("ACTIVE"))
    ...
    void authenticatedUsersCanReadOnlyActiveDirectoryEntries() throws Exception {
        ...
        mvc.perform(get("/api/v1/users/directory").with(user))
                ...
                .andExpect(jsonPath("$.error.name").value("Không có quyền truy cập"))
                .andExpect(jsonPath("$.error.message").value("Bạn không có quyền thực hiện thao tác này."));

        mvc.perform(get("/api/v1/users/directory"))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.error.code").value("unauthorized"))
                .andExpect(jsonPath("$.error.name").value("Phiên đăng nhập không hợp lệ"))
                .andExpect(jsonPath("$.error.message").value("Vui lòng đăng nhập lại."))
    ...
    void rootRoleFromIssuedJwtAuthorizesAdminApi() throws Exception {
        ...
        account.setRole(com.projectos.backend.identity.user.UserAccount.Role.ROOT_ADMIN);
