package vn.uytinmang.projectos.attendance.config;
...
class SecurityConfig {
    ...
    @Bean SecurityFilterChain attendanceSecurity(HttpSecurity http, JwtDecoder decoder, ApiSecurityErrorHandler errors) throws Exception {
        ...
        http.csrf(csrf -> csrf.disable())
                .addFilterBefore(new CookieCsrfFilter("PROJECT_OS_ACCESS"), BearerTokenAuthenticationFilter.class)
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .exceptionHandling(exceptions -> exceptions.authenticationEntryPoint(errors).accessDeniedHandler(errors))
                .authorizeHttpRequests(authorize -> authorize.requestMatchers("/actuator/health", "/v3/api-docs/**").permitAll().anyRequest().authenticated())
                .oauth2ResourceServer(resource -> resource.bearerTokenResolver(new CookieBearerTokenResolver("PROJECT_OS_ACCESS"))
